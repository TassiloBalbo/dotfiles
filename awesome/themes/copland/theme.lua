...
local mytextclock = wibox.widget.textclock("<span font='Hack 5'> </span>%H:%M ")
...
theme.mail = lain.widget.imap({
    ...
    settings = function()
        ...
        if mailcount > 0 then
            mail = "<span font='Hack 5'> </span>Mail "
            count = mailcount .. " "
        end
...
theme.mpd = lain.widget.mpd({
    settings = function()
        if mpd_now.state == "play" then
            title = mpd_now.title
            artist  = " " .. mpd_now.artist  .. markup("#777777", " <span font='Hack 2'> </span>|<span font='Hack 5'> </span>")
            mpdicon:set_image(theme.play)
        elseif mpd_now.state == "pause" then
            title = "mpd "
            artist  = "paused" .. markup("#777777", " |<span font='Hack 5'> </span>")
...
theme.fs = lain.widget.fs {
    notification_preset = { fg = theme.fg_normal, bg = theme.bg_normal, font = "Hack 10.5" },
    settings  = function()
...
-- Ping
theme.ping = awful.widget.watch('bash -c "ping -c 1 www.google.com | sed -n 2p | cut -d "=" -f 4"', 3)

-- Corona
theme.corona = awful.widget.watch('bash -c "/home/filip/.scripts/corona"', 10800)

-- Separators
local first     = wibox.widget.textbox(markup.font("Hack 3", " "))
local spr       = wibox.widget.textbox(' ')
local small_spr = wibox.widget.textbox(markup.font("Hack 4", " "))
local bar_spr   = wibox.widget.textbox(markup.font("Hack 3", " ") .. markup.fontfg(theme.font, "#81A1C1", "|") .. markup.font("Hack 5", " "))
