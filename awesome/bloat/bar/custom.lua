local panel = require "bloat.pop.panel"
local notif = require "bloat.pop.notif"
-- wibar.lua
-- Wibar (top bar)
...
awesome_icon:buttons(
	awful.util.table.join(
		awful.button({}, 1, function()
			if panelPop.visible then
				panelPop.visible = false
			else
				panelPop:move_next_to(mouse.current_widget_geometry)
			end
		end)
	)
)
...
-- open notifs on click
notif_icon:buttons(
	awful.util.table.join(
		awful.button({}, 1, function()
			if notifPop.visible then
				notifPop.visible = false
			else
				notifPop:move_next_to(mouse.current_widget_geometry)
			end
		end)
	)
)
...
-- override function list_update for fixed width of items
